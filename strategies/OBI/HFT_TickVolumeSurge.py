import asyncio, json, time, sys
from blessed import Terminal
from websockets import connect
from collections import deque

# --- Configuração ---
SYMBOL = "tBTCUSD"
USD_BALANCE = 10000.0
BTC_BALANCE = 0.5
ORDER_SIZE = 0.02  # Slightly larger to capitalize on high-conviction surges
STOP_LOSS = 0.70
TAKE_PROFIT = 1.20
MAX_CONCURRENT_TRADES = 50 
SURGE_THRESHOLD = 3.0  # Trigger if update rate is 3x the average
TERM = Terminal()

class SurgeTrader:
    def __init__(self):
        self.usd = USD_BALANCE
        self.btc = BTC_BALANCE
        self.total_trades = 0
        self.gains = 0
        self.losses = 0
        self.active_trades = []
        self.book = {'bids': {}, 'asks': {}}
        self.status_msg = "SURGE ENGINE ACTIVE"
        self.start_time = time.time()
        self.last_full_clear = time.time()
        
        # Track time of every incoming WebSocket update
        self.tick_times = deque(maxlen=200) 
        self.last_price = 0

    def get_market_state(self):
        if not self.book['bids'] or not self.book['asks']: return None
        
        bids = sorted(self.book['bids'].items(), reverse=True)
        asks = sorted(self.book['asks'].items())
        
        best_bid = bids[0][0]
        best_ask = asks[0][0]
        mid = (best_bid + best_ask) / 2
        
        now = time.time()
        # Calculate Current Tick Rate (Updates per second over the last 100ms)
        recent_ticks = [t for t in self.tick_times if now - t < 0.1]
        tick_rate = len(recent_ticks) / 0.1
        
        # Directional bias (is price ticking up or down during this surge?)
        bias = 1 if mid > self.last_price else -1 if mid < self.last_price else 0
        self.last_price = mid
        
        return {
            'bid': best_bid, 'ask': best_ask, 
            'rate': tick_rate, 'bias': bias, 'mid': mid
        }

    async def logic_loop(self):
        # Average rate baseline
        avg_rate = 10.0 
        
        while True:
            m = self.get_market_state()
            if m and len(self.active_trades) < MAX_CONCURRENT_TRADES:
                # Update baseline slowly
                avg_rate = (avg_rate * 0.99) + (m['rate'] * 0.01)
                
                # TRIGGER: Surge detected (Rate > Baseline * Threshold)
                if m['rate'] > (avg_rate * SURGE_THRESHOLD):
                    # Buy if price is ticking up during surge
                    if m['bias'] > 0 and self.usd >= (ORDER_SIZE * m['bid']):
                        self.execute_trade('BUY', m['bid'])
                    # Sell if price is ticking down during surge
                    elif m['bias'] < 0 and self.btc >= ORDER_SIZE:
                        self.execute_trade('SELL', m['ask'])
                        
            await asyncio.sleep(0) 

    def execute_trade(self, side, price):
        if side == 'BUY':
            self.usd -= ORDER_SIZE * price
            self.btc += ORDER_SIZE
            self.active_trades.append({'entry': price, 'side': 'BUY'})
        else:
            self.usd += ORDER_SIZE * price
            self.btc -= ORDER_SIZE
            self.active_trades.append({'entry': price, 'side': 'SELL'})
        self.status_msg = f"SURGE DETECTED: {side} @ {price:.2f}"

    async def risk_controller(self):
        while True:
            m = self.get_market_state()
            if m and self.active_trades:
                for trade in list(self.active_trades):
                    current_p = m['ask'] if trade['side'] == 'BUY' else m['bid']
                    pnl = (current_p - trade['entry']) * (1 if trade['side'] == 'BUY' else -1)
                    if pnl >= TAKE_PROFIT or pnl <= -STOP_LOSS:
                        self.close_trade(trade, current_p, pnl)
            await asyncio.sleep(0)

    def close_trade(self, trade, price, pnl):
        if trade['side'] == 'BUY':
            self.usd += price * ORDER_SIZE
            self.btc -= ORDER_SIZE
        else:
            self.usd -= price * ORDER_SIZE
            self.btc += ORDER_SIZE
        self.total_trades += 1
        if pnl > 0: self.gains += 1
        else: self.losses += 1
        self.active_trades.remove(trade)

    async def ui_loop(self):
        while True:
            now = time.time()
            if now - self.last_full_clear > 0.5:
                sys.stdout.write(TERM.clear); self.last_full_clear = now
            m = self.get_market_state()
            if m:
                uptime = time.strftime("%H:%M:%S", time.gmtime(now - self.start_time))
                out = TERM.home
                out += TERM.bold_black_on_orange(f" HFT SURGE DETECTOR | UP: {uptime} ".center(TERM.width)) + "\n"
                out += f" BID: {m['bid']:.2f} | ASK: {m['ask']:.2f}\n"
                out += f" TICK RATE: {m['rate']:.1f} upd/s | {TERM.bold(f'BIAS: {m['bias']}')}\n"
                out += f" SALDO USD: ${self.usd:,.2f} | SALDO BTC: {self.btc:.6f}\n"
                out += f"{TERM.orange('─' * TERM.width)}\n"
                out += f" TRADES: {self.total_trades} | WIN RATE: {(self.gains/max(1,self.total_trades)*100):.1f}%\n"
                out += f" MSG: {self.status_msg[:50]}\n"
                sys.stdout.write(out); sys.stdout.flush()
            await asyncio.sleep(0.05)

    async def socket_handler(self):
        uri = "wss://api-pub.bitfinex.com/ws/2"
        async with connect(uri) as ws:
            await ws.send(json.dumps({"event":"subscribe","channel":"book","symbol":SYMBOL,"prec":"P0"}))
            while True:
                msg = await ws.recv()
                self.tick_times.append(time.time()) # Log update time
                data = json.loads(msg)
                if isinstance(data, list) and len(data) > 1 and isinstance(data[1], list):
                    update = data[1]
                    if isinstance(update[0], list):
                        for e in update: self.update_book(e)
                    else: self.update_book(update)

    def update_book(self, e):
        if len(e) < 3: return
        p, c, a = e
        s = 'bids' if a > 0 else 'asks'
        if c > 0: self.book[s][p] = a
        else: self.book[s].pop(p, None)

async def main():
    bot = SurgeTrader()
    with TERM.fullscreen(), TERM.hidden_cursor():
        await asyncio.gather(bot.socket_handler(), bot.logic_loop(), bot.risk_controller(), bot.ui_loop())

if __name__ == "__main__":
    try: asyncio.run(main())
    except KeyboardInterrupt: pass
